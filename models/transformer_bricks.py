...
from .networks.ms_deform_attn_func import MSDeformAttnFunction
...
class MSDeformableAttention3D(nn.Module):
    ...
    def init_weights(self):
        ...
        # nn.init.kaiming_uniform_(self.sampling_offsets)
        thetas = torch.arange(
            self.num_heads,
        ...
        # nn.init.constant_(self.attention_weights, val=0.)
    ...
    def forward(self,
        ...
        output = MSDeformAttnFunction.apply(
            ...
            attention_weights, self.im2col_step).contiguous()
        # else:
...
#         return query
class LATRTransformerDecoderLayer(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.embed_dims = args._dim_
        self.MultiHeadAttention = nn.MultiheadAttention(num_heads=4, dropout=0.1, embed_dim=args._dim_)
        self.MSDeformableAttention3D = MSDeformableAttention3D(embed_dims=args._dim_, num_heads=4, num_levels=1, \
                                                               num_points=8, dropout=0.1, num_query=args.num_query, \
                                                                num_anchor_per_query=args.num_pt_per_line, anchor_y_steps=args.anchor_y_steps, \
        ...
        self.ffn = nn.Sequential(
            ...
            nn.Dropout(0.1),
            nn.Linear(self.embed_dims * 8, self.embed_dims),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
        )
        self.feedforward_channels=args._dim_*8
    ...
    def forward(self, query,
                ...
                **kwargs):
        norm_cnt = 0
        tem_key = tem_value = key
        query = self.MultiHeadAttention(query, tem_key, tem_value, key_padding_mask=key_padding_mask, need_weights=False)
        query = query[0]
        input_residual = query
        query = self.norm1(query)
        query = self.MSDeformableAttention3D(query, key, value, identity=input_residual, query_pos=query_pos, \
                                             key_pos=key_pos, attn_masks=attn_masks, key_padding_mask=key_padding_mask,
                                             **kwargs) 
        query = query + input_residual
        input_residual = query
...
class LATRTransformerDecoder(nn.Module):
    def __init__(self,
                 ...
                 anchor_y_steps=None,
                 **kwargs):
        super(LATRTransformerDecoder, self).__init__()
        
        self.num_layers = args.num_decoder_layers
        embed_dims = args._dim_
        num_query = args.num_query
        num_anchor_per_query = 20
        anchor_y_steps = args.anchor_y_steps

        self.embed_dims = embed_dims
        self.num_heads = 4
        self.feedforward_channels = self.embed_dims*8,
        self.dropout = 0.1
        self.layers = nn.ModuleList()
        for i in range(self.num_layers):
            layer = LATRTransformerDecoderLayer(args)
    ...
    def init_weights(self):
        for l in self.gflat_pred_layer:
            nn.init.xavier_uniform_(l, gain=0.01)
...
class LATRTransformer(nn.Module):
    def __init__(self, args):
        super(LATRTransformer, self).__init__()
        self.encoder = None
        self.decoder = LATRTransformerDecoder(args)
        self.embed_dims = args._dim_
        self.init_weights()

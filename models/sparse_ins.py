...
class MaskBranch(nn.Module):
    def __init__(self, args, in_channels):
        super().__init__()
        dim = args._dim_
        num_convs = args.sparse_ins_num_convs
        kernel_dim = args._dim_
...
class InstanceBranch(nn.Module):
    def __init__(self, args, in_channels, **kwargs):
        super().__init__()
        num_mask = args.num_query
        dim = args._dim_
        num_classes = args.num_category
        kernel_dim = args._dim_
        num_convs = args.sparse_ins_num_convs
        num_group = args.sparse_ins_num_groups
        sparse_num_group = args.sparse_ins_sparse_num_groups
    ...
    def forward(self, seg_features, is_training=True):
        ...
        iam = self.iam_conv(seg_features.tile((1, self.num_group, 1, 1)))
        if not is_training:
            iam = iam.view(
                iam.shape[0],
                self.num_group,
        ...
        # aggregate features: B*C*H*W -> B*N*C
        # (B x N x HW) @ (B x HW x C) -> B x N x C
        all_inst_features = torch.bmm(
            iam_prob_norm_hw, seg_features.view(B, C, -1).permute(0, 2, 1)
        )
...
class SparseInsDecoder(nn.Module):
    def __init__(self, args, **kargs) -> None:
        super().__init__()
        in_channels = args.sparse_ins_encoder_out_dims + 2
        self.output_iam = args.sparse_ins_decoder_output_iam
        self.scale_factor = args.sparse_ins_decoder_scale_factor
        self.sparse_decoder_weight = args.sparse_ins_sparse_decoder_weight

        self.mask_branch = MaskBranch(args, in_channels)
        self.inst_branch = InstanceBranch(args, in_channels)
    ...
    def compute_coordinates(self, x):
        ...
        y_loc = -1.0 + 2.0 * torch.arange(h, device=x.device) / (h - 1) # [-1, 1]
        x_loc = -1.0 + 2.0 * torch.arange(w, device=x.device) / (w - 1)
        y_loc, x_loc = torch.meshgrid(y_loc, x_loc)
        y_loc = y_loc.expand([x.shape[0], 1, -1, -1]) # [B, 1, H, W]
        x_loc = x_loc.expand([x.shape[0], 1, -1, -1])
        locations = torch.cat([x_loc, y_loc], 1) # [B, 2, H, W]
    ...
    def forward(self, features, is_training=True, **kwargs):
        ...
        if is_training:
            ...
            output.update(dict(
                pred_masks=pred_masks)) # Instance Segmentation Map
            
        if is_training:
            sparse_inst_losses, matched_indices = self.loss(
                output,
                lane_idx_map = kwargs.get('lane_idx_map', None),
